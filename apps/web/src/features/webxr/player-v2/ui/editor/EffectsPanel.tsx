...
import { dispatchWebXrTimelineEvent } from "@/features/webxr/pc-editor/data/timeline-bridge";
import { WEBXR_PC_EFFECT_PREVIEW_EVENT, type PcEffectPreviewDetail } from "@/features/webxr/pc-editor/ui/PcEffectPreview";
...
const EFFECT_CATEGORIES: EffectCategory[] = [
  ...
  },
  {
    ...
    label: "Frame",
    effects: [
      { durationMs: 900, eventName: "frame.hero_push", id: "hero-push", key: "1", label: "Hero push", params: { curve: "easeOutBackSoft", deltaFovH: -10, peakAtRatio: 0.72, reboundFovH: 1 } },
      { durationMs: 1400, eventName: "frame.reveal_pull", id: "reveal-pull", key: "2", label: "Reveal pull", params: { curve: "easeInOutCubic", deltaFovH: 14, deltaPitch: 2 } },
      ...
      { durationMs: 1600, eventName: "frame.little_planet_pullback", id: "little-planet", key: "7", label: "Little planet", params: { peakAtMs: 560, peakPitch: -88, peakSphereFov: 175, previewFlightHeight: 46.8, previewFov: 138, previewPitch: -90 } },
  ...
  },
  {
    ...
    label: "Glitch",
    effects: [
      { durationMs: 520, eventName: "filter.chromatic_aberration", id: "rgb-split", key: "1", label: "RGB split", params: { edgeMs: 110, offsetPx: 14, strength: 0.88 } },
...
export function EffectsPanel() {
  ...
  return (
    <>
      <aside className={collapsed ? `${styles.effectsPanel} ${styles.collapsed}` : styles.effectsPanel} data-testid="effects-panel">
        <div className={styles.effectsHeader}>
          ...
          <div>
            <div className={styles.workbenchChrome} aria-hidden="true">
              <span className={styles.dotMagenta} />
              <span className={styles.dotCyan} />
              <span className={styles.dotOrange} />
            </div>
            <p className={styles.workbenchKicker}>Tab effects</p>
            <h2>Effects Rack</h2>
          </div>
        </div>
        <div aria-hidden={collapsed} className={styles.effectsBody}>
          {categories.map((category) => {
            const open = openCategories.has(category.id);
            return (
              <section className={styles.effectCategory} data-active={activeCategory?.id === category.id} key={category.id}>
                <button
                  aria-expanded={open}
                  className={styles.effectCategoryButton}
                  onClick={() =>
                  ...
                  type="button"
                >
                  <span className={styles.effectKey}>{category.key}</span>
                ...
                {open ? (
                  <div className={styles.effectGrid}>
                    {category.effects.map((effect) => (
                      <button
                        className={activeEffect?.id === effect.id ? `${styles.effectTile} ${styles.active}` : styles.effectTile}
                        data-testid={`effect-${category.id}-${effect.id}`}
                        key={effect.id}
                        onClick={() => selectEffect(category, effect)}
                        type="button"
                      >
                        <span className={styles.effectBadge}>{effect.key}</span>
      ...
      {shortcut.mode !== "hidden" ? (
        <div className={styles.effectShortcutOverlay} data-testid="effect-shortcut-overlay">
          <div className={styles.effectShortcutCard}>
            <p className={styles.workbenchKicker}>
