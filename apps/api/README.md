...
`app.timeline_assembler` is the first pure-Python implementation slice for `ViewPathTimeline`. It can compile linear WebXR points plus effect events into a timeline dictionary with `editSegments`, `viewTracks`, `effectTracks`, `effectSystem`, coverage gaps, and build warnings. It is not wired to an HTTP export endpoint yet.
