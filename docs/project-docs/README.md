...
00-overview/pc-editor-webxr-effects-problem-analysis.md
PC Editor WebXR 特效系统规划与实现：问题、现状架构、黑场范例、后续特效扩展原则。

00-overview/pc-editor-webxr-speed-clock-mechanism.md
PC Editor WebXR 三速度与暂停机制：播放倍速、录制倍速、视效倍速、子弹时间和暂停 gate 的状态机语义。

00-overview/pc-editor-webxr-timeline-overwrite-and-speed-mechanism.md
PC Editor WebXR Timeline 覆写与三速度机制：重复播放覆盖、暂停 gate、录制倍速 editSegments、视效局部 timeline 和后端渲染对齐。

00-overview/pc-editor-webxr-effects-catalog-and-roadmap.md
特效分类、特效清单、分类型实现路线，以及后端平面视频开源包调研。

00-overview/pc-editor-webxr-advanced-camera-presets-blueprint.md
高级运镜预设蓝本：记录小行星前端 WebXR 近似、后端 equirect 重投影链路，以及 Crystal Ball / Slow Rise 扩展模板。
...
后端按注册登录、上传下载、视频列表、WebXR 桥接、Timeline 数据和视频裁切拆分后的预期设计。
...
后端按注册登录、上传下载、视频列表、WebXR 桥接、Timeline 数据和视频裁切拆分后的当前状态。
