...
WebXR 已拆成 legacy Three.js 基线、桌面 lab、A-Frame 登录页和 A-Frame 播放器原型。
...
/xr/videos/:videoId/session/:sessionId 仍没有接入真实 A-Frame 业务播放器和 timeline bridge。
...
WebXR：
A-Frame 播放器、空间控件、取景遮罩和 timeline bridge 骨架已出现；真实业务 session 裁剪交互未完成。
...
2. 把 sourceUrl、videoId、sessionId 传入 AFrameVideoSpherePlayer。
3. 开启 enableTimelineBridge，让真实 head-gaze / controller-ray 输出 ViewPathPatch。
4. 保持播放器层和剪辑语义层分离。
5. 接入最小剪辑 UI：cut、discard/restore、FOV、manual flush。
6. 后端继续强化 patch 校验。
7. 再推进 60 秒分片队列。
```
