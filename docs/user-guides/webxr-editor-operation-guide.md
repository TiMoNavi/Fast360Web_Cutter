...
| 端 | 路由 | 用途 |
| --- | --- | --- |
| PC editor | `/xr/player`，例如 `http://127.0.0.1:3080/xr/player` | 桌面键鼠编辑、验证时间线、调试完整 PC workflow |
| 头显 / Three Official | `/xr/three-official-interactive-lab` | Quest / WebXR 空间 UI 实验，使用头显视角和手柄验证主要剪辑动作 |

后期产品入口收敛为 `/xr/player`。`/xr/videos/:videoId/session/:sessionId` 只作为当前兼容深链或测试入口，不再作为长期操作路线。
...
| FOV | Q/E 或 PC UI | `right stick up/down`；拖拽取景时也可 `right grip + right stick` |
...
FOV 在头显端不使用单独按钮直接改变，当前统一用右摇杆连续调整，范围为 35 到 110：

```text
right stick up/down -> 连续调整 FOV
right grip hold + right stick up/down -> 拖拽取景时同步调整 FOV
...
| 头显 | `right stick up/down`；拖拽取景时可 `right grip + right stick up/down` |
...
| right stick up/down | 调整 FOV，范围 35 到 110 |
| right grip hold + right stick up/down | 拖拽取景时同步调整 FOV |
| left grip hold | 进入 mask opacity modifier |
...
为了避免误触，FOV 不放到 quick menu 或单独按钮里，而是使用 right stick 连续调整。拖拽取景时可以继续按住 right grip 并用 right stick 同步调 FOV。
