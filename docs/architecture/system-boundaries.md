...
本目录说明三端边界和共享系统约束。完整产品与协议总方案见 [`overview.md`](./overview.md)，黑盒业务边界见 [`black-box-boundaries.md`](./black-box-boundaries.md)，分端实施文档见 `../specs/`。
...
后端：
FastAPI + Pydantic + SQLite + 本地文件 + UploadFile + OpenCV remap + FFmpeg 编解码

后端后续：
RQ / Redis 或同类任务队列、tusd、PostgreSQL、对象存储

后端后续优化：
PyAV / GPU remap，参考 py360convert
```
